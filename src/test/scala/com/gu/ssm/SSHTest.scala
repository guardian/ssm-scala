  ...
  "create ssh command" - {
    ...
    "create standard ssh command" - {
      ...
      "instance id is correct" in {
        val (instanceId, _) = sshCmdStandard(false)(file, instance, "user4", "34.1.1.10", None, None, false)
        instanceId.id shouldEqual "raspberry"
      }

      "user command" - {
        "is correctly formed without port specification" in {
          val (_, command) = sshCmdStandard(false)(file, instance, "user4", "34.1.1.10", None, None, false)
          command should include ("""ssh -o "IdentitiesOnly yes" -i /banana user4@34.1.1.10""")
        }

        "is correctly formed with port specification" in {
          val (_, command) = sshCmdStandard(false)(file, instance, "user4", "34.1.1.10", Some(2345), None, false)
          command should include ("""ssh -o "IdentitiesOnly yes" -p 2345 -i /banana user4@34.1.1.10""")
        }

        "is correctly formed with a hosts file" in {
          val (_, command) = sshCmdStandard(false)(file, instance, "user4", "34.1.1.10", Some(2345), Some(new File("/tmp/hostsfile")), false)
          command should include ("""ssh -o "IdentitiesOnly yes" -o "UserKnownHostsFile /tmp/hostsfile" -o "StrictHostKeyChecking yes" -p 2345 -i /banana user4@34.1.1.10""")
        }

        "is correctly formed with agent forwarding file" in {
          val (_, command) = sshCmdStandard(false)(file, instance, "user4", "34.1.1.10", Some(2345), None, true)
      ...
      "machine command" - {
        "is correctly formed without port specification" in {
          val (_, command) = sshCmdStandard(true)(file, instance, "user4", "34.1.1.10", None, None, false)
          command should equal ("""ssh -o "IdentitiesOnly yes" -i /banana -t -t user4@34.1.1.10""")
        }

        "is correctly formed with port specification" in {
          val (_, command) = sshCmdStandard(true)(file, instance, "user4", "34.1.1.10", Some(2345), None, false)
    ...
    "create bastion ssh command" - {
      ...
      "instance id is correct" in {
        val (instanceId, _) = sshCmdBastion(false)(file, bastionInstance, targetInstance, "user5", "34.1.1.10", "10.1.1.11", None, "bastionuser", None, false, None)
        instanceId.id shouldEqual "strawberry"
      }

      "user command" - {
        "contains the user instructions" in {
          val (_, command) = sshCmdBastion(false)(file, bastionInstance, targetInstance, "user5", "34.1.1.10", "10.1.1.11", None, "bastionuser", None, false, None)
      ...
      "machine command" - {
        "is well formed without any port specification" - {
          "no agent" in {
            val (_, command) = sshCmdBastion(true)(file, bastionInstance, targetInstance, "user5", "34.1.1.10", "10.1.1.11", None, "bastionuser", None, false, None)
          ...
          "with agent" in {
            val (_, command) = sshCmdBastion(true)(file, bastionInstance, targetInstance, "user5", "34.1.1.10", "10.1.1.11", None, "bastionuser", None, true, None)
        ...
        "is well formed with target instance port specification" - {
          "no agent" in {
            val (_, command) = sshCmdBastion(true)(file, bastionInstance, targetInstance, "user5", "34.1.1.10", "10.1.1.11", None, "bastionuser", Some(2345), false, None)
          ...
          "with agent" in {
            val (_, command) = sshCmdBastion(true)(file, bastionInstance, targetInstance, "user5", "34.1.1.10", "10.1.1.11", None, "bastionuser", Some(2345), true, None)
        ...
        "is well formed with bastion port specification" - {
          "no agent" in {
            val (_, command) = sshCmdBastion(true)(file, bastionInstance, targetInstance, "user5", "34.1.1.10", "10.1.1.11", Some(1234), "bastionuser", None, false, None)
          ...
          "with agent" in {
            val (_, command) = sshCmdBastion(true)(file, bastionInstance, targetInstance, "user5", "34.1.1.10", "10.1.1.11", Some(1234), "bastionuser", None, true, None)
        ...
        "is well formed with both bastion port and target instance port specifications" - {
          "no agent" in {
            val (_, command) = sshCmdBastion(true)(file, bastionInstance, targetInstance, "user5", "34.1.1.10", "10.1.1.11", Some(1234), "bastionuser", Some(2345), false, None)
          ...
          "with agent" in {
            val (_, command) = sshCmdBastion(true)(file, bastionInstance, targetInstance, "user5", "34.1.1.10", "10.1.1.11", Some(1234), "bastionuser", Some(2345), true, None)
        ...
        "is well formed with a host key file" in {
          val (_, command) = sshCmdBastion(true)(file, bastionInstance, targetInstance, "user5", "34.1.1.10", "10.1.1.11", Some(1234), "bastionuser", Some(2345), false, Some(new File("/tmp/hostfile")))
          command should equal ("""ssh -o "IdentitiesOnly yes" -i /banana -o "UserKnownHostsFile /tmp/hostfile" -o "StrictHostKeyChecking yes" -o 'ProxyCommand ssh -o "IdentitiesOnly yes" -i /banana -o "UserKnownHostsFile /tmp/hostfile" -o "StrictHostKeyChecking yes" -p 1234 bastionuser@34.1.1.10 nc 10.1.1.11 2345' -t -t user5@10.1.1.11""")
        }
      }
    }
  }
