...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    argParser.parse(args, Arguments.empty()) match {
      case Some(Arguments(Some(executionTarget), toExecuteOpt, Some(profile), region, Some(mode), Some(user), sism, _, _, usePrivate)) =>
        ...
        mode match {
          ...
          case SsmSsh =>
            setUpSSH(awsClients, profile, region, executionTarget, user, sism, usePrivate)
        }
      case Some(_) => fail()
  ...
  private def setUpSSH(awsClients: AWSClients, profile: String, region: Region, executionTarget: ExecutionTarget, user: String, sism: SingleInstanceSelectionMode, usePrivate: Boolean): Unit = {
